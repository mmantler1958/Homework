...
// Create list of phone numbers of users
function extractPhoneNumbers(usersArray) {
  const phoneNumbers = [];
...
// Print list phone numbers of users
console.log(extractPhoneNumbers(users).join(" | "));

// Create list of website and email pairs of users
function extractWebsiteEmail(usersArray) {
  const websiteEmail = [];
  usersArray.forEach((user) => {
    websiteEmail.push("www=" + user.website + " email=" + user.email);
  });
  return websiteEmail;
}

// Print list of website and email pairs of users
console.log(extractWebsiteEmail(users).join(" | "));

const userIdNineUsers = toDos.filter((user) => (user.userId = 9));
console.log("User ID as 9:", userIdNineUsers);
const User9IncompleteTasks = userIdNineUsers.filter((user) => user.completed);
console.log(
  "Count of User ID 9 as incomplete:",
  User9IncompleteTasks.length - 1
);

//Create list of user 5's posts
const user5Posts = userPosts.filter((post) => (post.userId = 5));

console.log(user5Posts);

//create a list of title, body pairs
const titleBodyPairs = user5Posts.map(
